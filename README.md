...
| `agent` | `plugins/agent/` | `PERMISSION_STORAGE`, `PERMISSION_EVENT_PUBLISH` (+ operator JWT grant for dispatched actions) | Multi-step goal loop over `ai`'s `chat_completion`: tool specs auto-discovered from registered plugins' manifests (kernel read-only `list_plugins`/`get_manifest`, no admin needed) gated by an operator allowlist; confirmation-marked tools halt until `goal_resume {approve}`; goals persist as `goal:<id>` docs in `database`. See `plugins/agent/README.md`. |
...
| `filesystem` | `plugins/filesystem/` | `PERMISSION_FILES_READ`, `PERMISSION_FILES_WRITE` | Sandboxed local file browse/read/write: `fs_list` (directory browse), `fs_read` (windowed read), `fs_write` (create/overwrite) against an operator-defined root allowlist — path escapes are rejected (`ERR_FILES_PATH_ESCAPES_ROOT`). No exec, no shell, no delete/rename. See `plugins/filesystem/README.md`. |
