...
use std::sync::Arc;
use rss_plugin::{handle_action, Config, Rpc, RpcCall};
...
fn manifest() -> PluginManifest {
    PluginManifest { permissions: vec!["PERMISSION_STORAGE".into(), "PERMISSION_NETWORK".into()], actions: ACTIONS.iter().map(|s| s.to_string()).collect(), ..Default::default() }
}
    ...
    loop {
        tokio::select! {
            env = client.recv() => {
                ...
                match env.payload {
                    Some(envelope::Payload::Ping(p)) => { let pong = Envelope { payload: Some(envelope::Payload::Pong(Pong { original_timestamp: p.timestamp, server_timestamp: unix_millis() })), ..Default::default() }; let _ = client.send("kernel", pong).await; }
                    ...
                    Some(envelope::Payload::ActionRequest(req)) => {
                        ...
                        tokio::spawn(async move {
                            match handle_action(rpc, &cfg, &req.action, &req.params_json, start).await {
                                Ok(res) => { let _ = out.send(action_response(req.action_id, ActionStatus::ActionOk, res.data, String::new())).await; if let Some((t,p)) = res.event { let _ = out.send(Envelope { payload: Some(envelope::Payload::EventPublish(vynkor_sdk::proto::EventPublish { event_type: t, payload_json: p.to_string().into_bytes() })), ..Default::default() }).await; } }
            ...
            Some(call) = rpc_rx.recv() => {
                seq+=1; let action_id = format!("rpc-{seq}"); pending.insert(action_id.clone(), (call.action.clone(), call.reply));
...
mod tests {
    ...
    #[derive(Default)] struct FakeDb { kv: BTreeMap<String, Value> }
    impl FakeDb {
        fn handle(&mut self, action: &str, params: Value) -> Result<Value, String> {
            ...
            match action {
                ...
                "http_request" => Ok(serde_json::json!({"status":200,"body": r#"<?xml version="1.0"?><rss version="2.0"><channel><title>Test</title><item><title>Hello</title><link>https://example.com/1</link><pubDate>Mon, 01 Jan 2024 00:00:00 +0000</pubDate></item></channel></rss>"#, "body_encoding":"utf8"})),
    ...
    async fn run_shim(mut kernel: VynkorClient, mut rx: mpsc::Receiver<Cmd>) {
        ...
        loop {
            ...
            if let Some(envelope::Payload::PluginRegister(_))=env.payload {
                let _=kernel.send("rss", Envelope { payload: Some(envelope::Payload::PluginRegisterAck(PluginRegisterAck { accepted:true, ..Default::default() })), ..Default::default() }).await;
                break;
            }
        }
        loop {
                ...
                env = kernel.recv() => {
                    ...
                    match env.payload {
                        Some(envelope::Payload::ActionRequest(req)) => {
                            ...
                            let _=kernel.send("rss", Envelope { payload: Some(envelope::Payload::ActionResponse(resp)), ..Default::default() }).await;
                        }
                        ...
                        Some(envelope::Payload::EventPublish(ev)) => { let _=kernel.send("rss", Envelope { payload: Some(envelope::Payload::EventPublishAck(EventPublishAck { event_id: format!("ev-{seq}"), status: EventPublishStatus::EventPublishOk as i32, error: String::new() })), ..Default::default() }).await; seq+=1; let _=ev; }
                        Some(envelope::Payload::Ping(p)) => { let _=kernel.send("rss", Envelope { payload: Some(envelope::Payload::Pong(Pong { original_timestamp: p.timestamp, server_timestamp: unix_millis() })), ..Default::default() }).await; }
                        _=>{}
                    }
                }
                cmd = rx.recv() => {
                    match cmd {
                        Some(Cmd::Call { action, params, reply }) => { seq+=1; let id=format!("t-{seq}"); pending.insert(id.clone(), reply); let env=Envelope { payload: Some(envelope::Payload::ActionRequest(ActionRequest { action_id: id, action, params_json: serde_json::to_vec(&params).unwrap(), timeout_ms:0, streaming:false, ..Default::default() })), ..Default::default() }; let _=kernel.send("rss", env).await; }
                        None=>break,
                    }
                }
            }
    ...
    fn test_cfg() -> Config { Config { db_timeout_ms: 5000, fetch_timeout_ms: 5000 } }
    #[tokio::test]
    async fn add_list_remove() {
        let shim=start_plugin(test_cfg()).await;
        let added=shim.call("rss_add", serde_json::json!({"url":"https://example.com/feed.xml"})).await.unwrap();
        assert_eq!(added["feed"]["url"], "https://example.com/feed.xml");
        let list=shim.call("rss_list", serde_json::json!({})).await.unwrap();
        assert_eq!(list["feeds"].as_array().unwrap().len(), 1);
        let id=added["id"].as_str().unwrap();
        let rem=shim.call("rss_remove", serde_json::json!({"id": id})).await.unwrap();
        assert_eq!(rem["removed"], true);
    }
    #[tokio::test]
    async fn fetch_and_articles() {
        let shim=start_plugin(test_cfg()).await;
        let added=shim.call("rss_add", serde_json::json!({"url":"https://example.com/feed.xml"})).await.unwrap();
        let id=added["id"].as_str().unwrap();
        let res=shim.call("rss_fetch", serde_json::json!({"id": id})).await.unwrap();
        assert_eq!(res["fetched"], 1);
        assert_eq!(res["new"], 1);
        let arts=shim.call("rss_articles", serde_json::json!({})).await.unwrap();
        assert_eq!(arts["total"], 1);
        assert_eq!(arts["articles"][0]["title"], "Hello");
        // second fetch should not duplicate
        let res2=shim.call("rss_fetch", serde_json::json!({"id": id})).await.unwrap();
        assert_eq!(res2["new"], 0);
    }
    #[tokio::test]
    async fn articles_query_and_mark_read() {
        let shim=start_plugin(test_cfg()).await;
        let added=shim.call("rss_add", serde_json::json!({"url":"https://example.com/feed.xml"})).await.unwrap();
        let id=added["id"].as_str().unwrap();
        shim.call("rss_fetch", serde_json::json!({"id": id})).await.unwrap();
        let arts=shim.call("rss_articles", serde_json::json!({"query":"Hello"})).await.unwrap();
        assert_eq!(arts["total"], 1);
        let aid=arts["articles"][0]["id"].as_str().unwrap();
        let upd=shim.call("rss_mark_read", serde_json::json!({"id": aid, "read": true})).await.unwrap();
        assert_eq!(upd["updated"], true);
        let unread=shim.call("rss_articles", serde_json::json!({"unread_only": true})).await.unwrap();
