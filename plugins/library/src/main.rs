...
async fn serve(mut client: VynkorClient, config: Config) -> Result<(), VynkorError> {
    ...
    let ack = client
        .register_full(PLUGIN_ID, PLUGIN_VERSION, manifest(), &jwt)
        .await?;
    if !ack.accepted {
                                ...
                                Ok(res) => {
                                    ...
                                    if let Some((t, p)) = res.event {
                                        let _ = out.send(Envelope {
                                            payload: Some(envelope::Payload::EventPublish(vynkor_sdk::proto::EventPublish {
                                                event_type: t,
                                                payload_json: p.to_string().into_bytes(),
                                            })),
                                            ..Default::default()
                                        }).await;
                                    }
...
mod tests {
    ...
    impl FakeDb {
        fn handle(&mut self, action: &str, params: Value) -> Result<Value, String> {
            ...
            match action {
                "db_incr" => {
                    let k = key(&params);
                    let d = params.get("delta").and_then(Value::as_i64).unwrap_or(1);
                    let cur = self.kv.get(&k).and_then(Value::as_i64).unwrap_or(0);
                    let nxt = cur + d;
                    self.kv.insert(k, serde_json::json!(nxt));
                    Ok(serde_json::json!({"ok": true, "value": nxt}))
                }
                "db_set" => {
                    let k = key(&params);
                ...
                "db_keys" => {
                    ...
                    let keys: Vec<&String> = self.kv.keys().filter(|k| k.starts_with(prefix)).collect();
                    ...
                    if let Some(keys) = params.get("keys").and_then(Value::as_array) {
                        for k in keys {
                            if let Some(k) = k.as_str() {
                                values.insert(k.to_string(), self.kv.get(k).cloned().unwrap_or(Value::Null));
                            }
                        }
                    }
                ...
                env = kernel.recv() => {
                    ...
                    match env.payload {
                        Some(envelope::Payload::ActionRequest(req)) => {
                            ...
                            let _ = kernel.send("library", Envelope { payload: Some(envelope::Payload::ActionResponse(resp)), ..Default::default() }).await;
                        }
                        Some(envelope::Payload::ActionResponse(r)) => {
                            if let Some(tx) = pending.remove(&r.action_id) {
                                let res = if r.status == ActionStatus::ActionOk as i32 {
                                    serde_json::from_slice(&r.data_json).map_err(|e| e.to_string())
                                } else {
                                    Err(r.error)
                                };
                                let _ = tx.send(res);
                        ...
                        Some(envelope::Payload::EventPublish(ev)) => {
                            let _ = kernel.send("library", Envelope { payload: Some(envelope::Payload::EventPublishAck(EventPublishAck { event_id: format!("ev-{seq}"), status: EventPublishStatus::EventPublishOk as i32, error: String::new() })), ..Default::default() }).await;
                            seq += 1;
                            let _ = ev;
                        }
                        Some(envelope::Payload::Ping(p)) => {
                            let _ = kernel.send("library", Envelope { payload: Some(envelope::Payload::Pong(Pong { original_timestamp: p.timestamp, server_timestamp: unix_millis() })), ..Default::default() }).await;
                        }
                        _ => {}
                    }
                }
                cmd = rx.recv() => {
                    match cmd {
                        Some(Cmd::Call { action, params, reply }) => {
                            ...
                            let _ = kernel.send("library", env).await;
                        }
                        None => break,
                    }
                }
    ...
    fn test_cfg() -> Config { Config { db_timeout_ms: 5000 } }
    #[tokio::test]
    async fn scan_empty_roots_error() {
        let shim = start_plugin(test_cfg()).await;
        let err = shim.call("library_scan", serde_json::json!({})).await.unwrap_err();
        assert!(err.contains("ERR_LIBRARY_NO_ROOTS"), "{err}");
    }
    #[tokio::test]
    async fn stats_empty() {
        let shim = start_plugin(test_cfg()).await;
        let res = shim.call("library_stats", serde_json::json!({})).await.unwrap();
        assert_eq!(res["total"], 0);
    }
    #[tokio::test]
    async fn search_and_get() {
        // Directly insert via db and search via plugin
        let shim = start_plugin(test_cfg()).await;
        // Insert a fake entry via underlying db not exposed, so test search on empty
        let res = shim.call("library_search", serde_json::json!({"query": "nothing"})).await.unwrap();
        assert_eq!(res["total"], 0);
        let notfound = shim.call("library_get", serde_json::json!({"id": "missing"})).await.unwrap();
        assert_eq!(notfound["found"], false);
    }
}
