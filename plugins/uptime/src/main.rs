...
use std::sync::Arc;
use uptime_plugin::{handle_action, scan_all, Config, Rpc, RpcCall};
...
fn manifest() -> PluginManifest {
    PluginManifest { permissions: vec!["PERMISSION_STORAGE".into(), "PERMISSION_NETWORK".into(), "PERMISSION_EVENT_PUBLISH".into()], actions: ACTIONS.iter().map(|s| s.to_string()).collect(), ..Default::default() }
}
...
fn event_envelope(t: &str, p: &Value) -> Envelope { Envelope { payload: Some(envelope::Payload::EventPublish(EventPublish { event_type: t.to_string(), payload_json: p.to_string().into_bytes() })), ..Default::default() } }
...
async fn serve(mut client: VynkorClient, config: Config) -> Result<(), VynkorError> {
    ...
    let mut seq: u64 = 0;
    let mut interval = tokio::time::interval(std::time::Duration::from_secs(config.interval_secs.max(5)));
    interval.tick().await;
    loop {
        tokio::select! {
            env = client.recv() => {
                    ...
                    Some(envelope::Payload::ActionRequest(req)) => {
                        ...
                        tokio::spawn(async move {
                            match handle_action(rpc, &cfg, &req.action, &req.params_json, start).await {
                                Ok(res) => { let _ = out.send(action_response(req.action_id, ActionStatus::ActionOk, res.data, String::new())).await; if let Some((t,p)) = res.event { let _ = out.send(event_envelope(&t, &p)).await; } }
...
mod tests {
    ...
    impl FakeDb {
        fn handle(&mut self, action: &str, params: Value) -> Result<Value, String> {
            ...
            match action {
                ...
                "http_request" => Ok(serde_json::json!({"status":200,"body":"ok","body_encoding":"utf8"})),
    ...
    async fn run_shim(mut kernel: VynkorClient, mut rx: mpsc::Receiver<Cmd>) {
        ...
        loop {
            ...
            if let Some(envelope::Payload::PluginRegister(_))=env.payload {
                let _=kernel.send("uptime", Envelope { payload: Some(envelope::Payload::PluginRegisterAck(PluginRegisterAck { accepted:true, ..Default::default() })), ..Default::default() }).await;
                break;
            }
        }
        loop {
                ...
                env = kernel.recv() => {
                    ...
                    match env.payload {
                        Some(envelope::Payload::ActionRequest(req)) => {
                            ...
                            let _=kernel.send("uptime", Envelope { payload: Some(envelope::Payload::ActionResponse(resp)), ..Default::default() }).await;
                        }
                        ...
                        Some(envelope::Payload::EventPublish(ev)) => { let _=kernel.send("uptime", Envelope { payload: Some(envelope::Payload::EventPublishAck(EventPublishAck { event_id: format!("ev-{seq}"), status: EventPublishStatus::EventPublishOk as i32, error: String::new() })), ..Default::default() }).await; seq+=1; let _=ev; }
                        Some(envelope::Payload::Ping(p)) => { let _=kernel.send("uptime", Envelope { payload: Some(envelope::Payload::Pong(Pong { original_timestamp: p.timestamp, server_timestamp: unix_millis() })), ..Default::default() }).await; }
                        _=>{}
                    }
                }
                cmd = rx.recv() => {
                    match cmd {
                        Some(Cmd::Call { action, params, reply }) => { seq+=1; let id=format!("t-{seq}"); pending.insert(id.clone(), reply); let env=Envelope { payload: Some(envelope::Payload::ActionRequest(ActionRequest { action_id: id, action, params_json: serde_json::to_vec(&params).unwrap(), timeout_ms:0, streaming:false, ..Default::default() })), ..Default::default() }; let _=kernel.send("uptime", env).await; }
                        None=>break,
                    }
                }
            }
    ...
    async fn add_list_remove() {
        ...
        let added=shim.call("uptime_add", serde_json::json!({"url":"https://example.com"})).await.unwrap();
        assert_eq!(added["target"]["url"], "https://example.com");
        let list=shim.call("uptime_list", serde_json::json!({})).await.unwrap();
        assert_eq!(list["targets"].as_array().unwrap().len(), 1);
        let id=added["id"].as_str().unwrap();
        let rem=shim.call("uptime_remove", serde_json::json!({"id": id})).await.unwrap();
        assert_eq!(rem["removed"], true);
    }
    #[tokio::test]
    async fn check_ok() {
        let shim=start_plugin(test_cfg()).await;
        let res=shim.call("uptime_check", serde_json::json!({"url":"https://example.com"})).await.unwrap();
    ...
    async fn history_empty() {
        let shim=start_plugin(test_cfg()).await;
        let h=shim.call("uptime_history", serde_json::json!({})).await.unwrap();
        assert_eq!(h["total"], 0);
    }
}
