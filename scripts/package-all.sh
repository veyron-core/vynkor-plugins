...
# signed with the maintainer key from VEYRON_SIGNING_KEY_FILE.
# Reads /tmp/opencode/package_list.tsv: dir<TAB>slug<TAB>name<TAB>desc<TAB>cat<TAB>tags<TAB>status
set -euo pipefail
KEY="${VEYRON_SIGNING_KEY_FILE:?set VEYRON_SIGNING_KEY_FILE}"
...
while IFS=$'\t' read -r dir slug name desc cat tags status; do
    ...
    if ! VEYRON_SIGNING_KEY_FILE="$KEY" "$REPO/scripts/package.sh" "${args[@]}"; then
